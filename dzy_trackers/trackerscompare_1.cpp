...
*/
    ...
    while (frame.data)
    {
        ...
        if(key == 's'){
            flag = 1;
            putText(frameDraw, "!Choose Target!", cv::Point(10, 20), FONT_HERSHEY_SIMPLEX,0.5, Scalar(255, 0, 0), 2);
            readvideo.IniRead(bboxGroundtruth, frameDraw, window_name, capture);
            Rect2f ecobbox(bboxGroundtruth.x, bboxGroundtruth.y, bboxGroundtruth.width, bboxGroundtruth.height);
            ecotracker.init(frame, ecobbox, parameters);
        }
...
*/
	    ...
	    if (okeco){
                rectangle(frameDraw, ecobbox, Scalar(0, 0, 255), 2);
	    }
	    else{
	        putText(frameDraw, "!!!NO Target!!!", cv::Point(10, 50), FONT_HERSHEY_SIMPLEX,
	    ...
	    ostringstream os;
	    os << float(fpseco);
	    putText(frameDraw, "FPS: " + os.str(), Point(10, 20), FONT_HERSHEY_SIMPLEX,
